...
export const searchMovies = async (query) => {
  try {
    ...
    const movies = await db
      ...
      .limit(50)
      .toArray();
    console.log("Search Movies found:", movies.length, query);
    if(movies && movies.length > 0) {
      return { success: true, message: "Movies fetched successfully!", data: movies };
    }
    else {
      return { success: false, message: "No Movies found!", data: [] };
    }
  } catch (error) {
    console.log("MongoDB fetch failed", error);
      return { success: false, message: "Error fetching movies!", data: [] };
